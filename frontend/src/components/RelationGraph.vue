<template>
    <div class="box" style="height: 90%; width: 100%; text-align: center;">
        ...
        <div style="float: right; padding-right: 20px;">
            ...
            进行搜索
            <el-button icon="el-icon-search" circle type="primary" size="small" @click="$router.replace('/result/'+currentNodeText)"/>
        </div>
    </div>
</template>
...
export default {
                ...
                "layouts": [
                    {
                        ...
                        "layoutName": "force",
                        "maxLayoutTimes": 7500,
                        "force_node_repulsion": 1.0,
                        "force_line_elastic": 1.25
                    }
                ]
    ...
    methods: {
        async showSeeksGraph() {
            let params = { rootText : this.root}
            await axios.post('http://localhost:8088/getKG', params)
            .then(
                response => {
                    graphJsonData = response.data
                }
            )
        ...
        onNodeClick(nodeObject, $event) {
            this.$refs.graphRef.getNodes().forEach(node => {
                if (nodeObject.targetNodes.includes(node) || node == nodeObject || node.id == 'a')
                    node.opacity = 1
                else
                    node.opacity = 0.4
            });
